...
void drawField() {
  ...
  // Draw grid and color areas based on moisture and pH
  int rows = 10;
  int cols = 10;
  float cellWidth = fieldWidth * scaleFactor / cols;
  float cellHeight = fieldHeight * scaleFactor / rows;

  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < cols; j++) {
      int index = i * cols + j;
      if (index < dataPointIndex) {
        float moistureLevel = moistureData[index];
        float pHLevel = pHData[index];
        fill(getColorBasedOnMoistureAndPH("crop", moistureLevel, pHLevel));
      } else {
        fill(0); // default color
      }
      rect(fieldX + j * cellWidth, fieldY + i * cellHeight, cellWidth, cellHeight);
    }
  }
}
...
int getColorBasedOnMoistureAndPH(String crop, float moistureLevel, float pHLevel) {
  boolean moistureInRange = checkMoistureInRange(crop, moistureLevel);
  boolean pHInRange = checkpHInRange(crop, pHLevel);

  if (!moistureInRange && !pHInRange) {
...
void draw() {
  ...
  if (cp5.getTab("default").isActive()) {
   ...
   else if(cp5.getTab("Field").isActive()) {
     drawFieldScreen();
     drawField();
    drawPointer();
   }
